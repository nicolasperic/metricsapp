        ...
        <div class="col-md-12">
            <div class="card shadow mb-8">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary" style="display:inline;">Subscription Settings</h6>
                </div>
                <div class="card-body">

                    <form method="POST" action="{{route('customer.portal')}}" style="float:right;">
                        @csrf
                        <button type="submit" class="link">Manage billing</button>
                    </form>
                    ...
                    <form method="POST" action="{{route('checkout')}}" >
                        @csrf
                        <button type="submit" class="link">Checkout</button>
                    </form>

                    @if($subscription)
                        Currently subscribed to: {{$subscription['plan']}}<br>
                        {{$subscription['price']}} por mes<br>
                        Status: {{$subscription['status']}}<br>
                        Trial ends at:{{$subscription['trial_ends_at']}} (show only when on trialing status)<br>
                        Ends at:{{$subscription['ends_at']}}<br>


                        <a href="{{route('billing.portal')}}" target="_blank">Manage Subscription</a>
